...
static int f_sys_pagefault_e(struct event_filler_arguments *args)
{
	...
	res = val_to_ring(args, args->event_info.fault_data.address, 0, false, 0);
	if (unlikely(res != PPM_SUCCESS))
		return res;

	res = val_to_ring(args, args->event_info.fault_data.regs->ip, 0, false, 0);
	if (unlikely(res != PPM_SUCCESS))
		return res;

	res = val_to_ring(args, args->event_info.fault_data.error_code, 0, false, 0);
	if (unlikely(res != PPM_SUCCESS))
		return res;
